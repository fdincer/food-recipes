...
export default function SearchRecipeItem({recipe}: { recipe: RecipeItem[]} ) {
    ...
    const renderedRandomRecipe = recipe.map( (r) => {
        ...
        return (
            <div className="flex items-center  rounded-xl m-2 bg-slate-100 border-2 border-slate-400 p-2 h-60" key={r.recipeName}>
                <div className="w-80">
                ...
                <div className="items-center text-center w-2/5">
                    <button onClick={handleClick} className="border-4 border-green-800 px-8 py-4 bg-green-600 hover:bg-green-800 hover:underline cursor-pointer text-white flex flex-row items-center w-fit text-lg mx-auto">
                        <p>View Recipe</p>
